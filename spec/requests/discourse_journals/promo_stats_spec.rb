...
describe "DiscourseJournals promo stats" do
  ...
  describe "POST /journals/promo/track" do
    ...
    it "records a click" do
      post "/journals/promo/track.json", params: { slide: "prism", event: "click" }

      expect(response.status).to eq(200)
      expect(DiscourseJournals::PromoStat.find_by(slide: "prism").clicks).to eq(1)
    end
    ...
    it "rejects an unknown slide or event without recording" do
      ...
      post "/journals/promo/track.json", params: { slide: "claw", event: "hover" }
      expect(response.status).to eq(400)
  ...
  describe "GET /admin/journals/promo_stats" do
    ...
    it "returns the daily report for admins" do
      DiscourseJournals::PromoStat.track!(slide: "claw", event: "impression")
      sign_in(admin)
